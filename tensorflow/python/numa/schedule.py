...
def run_on_nodes(*nodes):
    ...
    if len(nodes) == 0:
        ...
        op_res = LIBNUMA.numa_run_on_node_mask(bitmask)
        if op_res == -1:
            raise Exception("attempt to run on {res} failed")
...
def get_allowed_cpus_num():
    return LIBNUMA.numa_num_task_cpus()


def get_allowed_nodes_num():
